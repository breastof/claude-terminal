...
**terminal-manager.js** — Manages PTY sessions via tmux. CLI runs inside `tmux -L claude-terminal` sessions that survive server restarts/deploys. node-pty attaches lazily (on first client connect). Tracks WebSocket clients, handles session lifecycle (create/stop/resume/delete/rename). Persists metadata to `~/.sessions.json`. Watches file for cross-instance sync during blue-green deploy. Replay buffer: chunk-list bounded at 2 MiB total `data.length` (chunk-granularity prune — never mid-CSI cut) when `CT_RELIABLE_STREAMING=1`; legacy 2 MB string accumulator (`slice(-2_000_000)`) otherwise.

**Reliable-streaming protocol (CT_RELIABLE_STREAMING=1).** Each PTY chunk carries an absolute monotonic `seq` (cumulative bytes since session start). Live `output` ships as binary frames `[1B opcode 0x01][8B BE uint64 seq][UTF-8 payload]` to clients that advertise `binary_capable:true` in their hello, JSON `{type:"output",seq,data}` otherwise. On reconnect the client sends `{type:"hello", protocol_version:2, binary_capable, lastSeq}`; server FAST_PATHs from the chunk list when `lastSeq` is within the rolling window, else SLOW_PATHs via `tmux capture-pane -p -e -J -S - [-a]` snapshot wrapped with `\x1b[2J\x1b[H` + cursor-position. Both paths terminate with a single `{type:"replay_complete"}` barrier. Per-client async drain with 8 MiB `bufferedAmount` ceiling — clients past the ceiling get `close(4503,"lagging")` and reconnect via seq-resume. Protocol-level WS ping every 25 s; clients without a pong by the next tick are terminated. Old clients (no hello within 2 s) fall through to legacy buffer dump with no protocol error. Full spec: `agent-workflow/06-integration-plan-tmux.md` §2.
