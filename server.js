...
app.prepare().then(() => {
  ...
  function symphonyBroadcast(event) {
    ...
    for (const ws of symphonyClients) {
  ...
  server.on("upgrade", (request, socket, head) => {
    ...
    if (pathname === "/api/symphony-events") {
      const token = query.token;
      const decoded = token ? verifyJWT(token) : null;
      ...
      wssSymphony.handleUpgrade(request, socket, head, (ws) => {
        symphonyClients.add(ws);
        ws.send(JSON.stringify({ type: "connected" }));
        ws.on("close", () => symphonyClients.delete(ws));
      });
      return;
    }
