...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      if (rawReplyToId) {
        replyToId = parseInt(String(rawReplyToId), 10) || null;
      }
      const replyToIdRaw = formData.get("replyToId") as string | null;
      if (replyToIdRaw) replyToId = parseInt(replyToIdRaw, 10) || null;
      ...
      text = body.text || "";
      if (body.replyToId) {
        replyToId = parseInt(String(body.replyToId), 10) || null;
      }
      if (body.replyToId) replyToId = parseInt(body.replyToId, 10) || null;
    }
    ...
    const cm = getChatManager();
    const message = cm.sendMessage(payload.userId, text, attachments, replyToId);
