...
import { Settings, FileIcon, FolderIcon, ChevronRight } from "@/components/Icons";
          ...
          <div key={entry.path}>
            ...
            >
              {entry.type === "directory" ? (
                <ChevronRight className={`w-3 h-3 text-muted-fg flex-shrink-0 transition-transform ${expanded[entry.path] ? "rotate-90" : ""}`} />
              ) : (
            ...
            {expanded[entry.path] && (
              <div className="pl-4">
                {expanded[entry.path].length === 0 && (
                  <div className="px-2 py-1.5 text-xs text-muted italic">пусто</div>
                )}
