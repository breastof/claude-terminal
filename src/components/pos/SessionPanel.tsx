...
import { Pencil, Trash, Play, Pause, FolderIcon, FolderPlus, Volume2, VolumeX, ChevronRight } from "@/components/Icons";
...
export default function SessionPanel({
  ...
  const fetchSessions = useCallback(async () => {
    try {
      ...
      if (seenOnceRef.current) {
        // Звук играем только на реальный busy→idle, а не на busy→исчезла
        // (стоп/удаление). Иначе любой Stop сессии триггерил «динь» завершения.
        const anyFinished = [...prevBusyRef.current].some((id) => {
          if (curBusy.has(id)) return false;
          const stillThere = next.find((s) => s.sessionId === id);
          return !!stillThere && stillThere.isActive;
        });
  ...
  // Per-group "show all" state — по умолчанию в раскрытой группе показываем
  // только первую (приоритетную) сессию + selected, остальные — за кнопкой.
  const [expanded, setExpanded] = useState<Record<string, boolean>>(() => {
    if (typeof window === "undefined") return {};
    try {
      const raw = localStorage.getItem("sessionGroupExpanded");
      if (raw) return JSON.parse(raw);
    } catch {}
    return {};
  });
  ...
  const toggleExpanded = (key: string) => {
    setExpanded((e) => {
      ...
      try { localStorage.setItem("sessionGroupExpanded", JSON.stringify(next)); } catch {}
      return next;
    });
  };
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="h-14 px-3 flex items-center gap-2 border-b border-border">
        ...
        <button
          onClick={toggleMute}
          className="p-2 rounded text-muted-fg hover:text-foreground hover:bg-surface-hover transition-colors flex-shrink-0"
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-3">
        ...
        {groups.map((group, idx) => {
          ...
          // Видимые сессии в раскрытой группе:
          //   isExpanded → все
          //   иначе → первая (приоритет: активные сверху по сортировке) + selected (если она в этой группе)
          // Видимые в раскрытой группе:
          //   isExpanded → все
          //   иначе → все активные + selected (если она в этой группе и не активная)
          //   fallback: если активных нет и selected нет — показываем первую,
          //   чтобы группа не выглядела пустой
          let visibleSessions = group.sessions;
          let hiddenCount = 0;
          if (!isExpanded && group.sessions.length > 1) {
            const visible = group.sessions.filter((s) => s.isActive);
            const selectedHere = group.sessions.find((s) => s.sessionId === activeSessionId);
            if (selectedHere && !visible.includes(selectedHere)) {
              visible.push(selectedHere);
            }
            if (visible.length === 0) {
              visible.push(group.sessions[0]);
            }
            visibleSessions = visible;
            hiddenCount = group.sessions.length - visibleSessions.length;
          }
          const newSessionInGroup = (e: React.MouseEvent) => {
            e.stopPropagation();
            if (group.isSandbox) {
              onNewSession(selectedSlug); // без dir → новая ~/projects/Claude/<id>
            } else {
              const dir = group.sessions[0]?.projectDir;
              if (dir) onNewSession(selectedSlug, dir);
            }
          };
          return (
            <div key={group.key}>
              <div className="px-2 py-2 flex items-center justify-between gap-2 group/header">
                <button
                ...
                >
                  <ChevronRight
                    className={`w-3.5 h-3.5 flex-shrink-0 text-muted-fg transition-transform ${isCollapsed ? "" : "rotate-90"}`}
                  />
                  <FolderIcon className="w-4 h-4 flex-shrink-0 text-muted-fg" />
                  <span className="text-sm font-semibold text-foreground truncate">
                    {group.label}
                  </span>
                  <span className="text-xs text-muted-fg font-normal flex-shrink-0">
                    {activeCount > 0 ? `${activeCount}/${group.sessions.length}` : group.sessions.length}
                  </span>
                </button>
                <button
                  onClick={newSessionInGroup}
                  disabled={creatingSession}
                  className="p-1 text-muted-fg hover:text-accent-fg hover:bg-surface-hover rounded transition-colors cursor-pointer flex-shrink-0 opacity-100 md:opacity-0 md:group-hover/header:opacity-100 disabled:opacity-50 disabled:cursor-not-allowed"
                  title={group.isSandbox ? "Новая сессия в песочнице" : `Новая сессия в проекте ${group.label}`}
                  aria-label="Новая сессия в проекте"
                >
                  <FolderPlus className="w-4 h-4" />
                </button>
              </div>
              {!isCollapsed && visibleSessions.map((session) => (
                <SessionItem
              ...
              {!isCollapsed && (hiddenCount > 0 || isExpanded) && (
                <button
                  ...
                  className="ml-7 px-2 py-0.5 text-xs text-muted-fg hover:text-foreground transition-colors cursor-pointer"
                >
                  {isExpanded ? "← скрыть" : `+ ещё ${hiddenCount}`}
                </button>
              )}
            </div>
          );
...
function SessionItem({
  ...
  // Visual hierarchy:
  //   слева:    точка состояния    — статус сессии (live/busy/waiting/stopped)
  //   фон:      bg + accent-полоса — выделена ты (selected)
  //   справа:   жирность + бейдж   — нужно твоё внимание (unread)
  // Stopped: opacity на всей строке + полая (outline-only) точка вместо заполненной.
  // Waiting (Claude просит permission) теперь отрисовывается так же, как busy,
  // но в амбер-цвете — раньше визуально не отличался от idle.
  return (
    <div
      onClick={onSelect}
      className={`relative px-3 py-3 md:py-2.5 rounded-lg transition-all duration-150 group cursor-pointer ${
    ...
    >
      {/* Selected indicator — accent-полоска слева */}
      {isSelected && (
        <span className="absolute left-0 top-2 bottom-2 w-[3px] rounded-r bg-accent-fg" />
      )}
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0 flex-1">
          {isResuming ? (
            <div className="w-3 h-3 flex-shrink-0 border-2 border-emerald-500/30 border-t-emerald-400 rounded-full animate-spin" />
          ) : (
            <span className="relative w-1.5 h-1.5 flex-shrink-0 inline-flex items-center justify-center">
              {session.isActive && (session.busy || session.waiting) && (
                <>
                  <span className={`absolute -inset-2 rounded-full animate-ping ${
                    session.busy ? "bg-emerald-400/40" : "bg-amber-400/40"
                  }`} />
                  <span className={`absolute -inset-1 rounded-full ${
                    session.busy ? "bg-emerald-400/30" : "bg-amber-400/30"
                  }`} />
                </>
              )}
              <span className={`relative w-1.5 h-1.5 rounded-full ${
                !session.isActive
                  ? "border border-muted-fg/60 bg-transparent"
                  : session.busy
          ...
          {isEditing ? (
            ...
            <span
              className={`text-sm truncate ${
                unread && session.isActive ? "font-semibold text-foreground" : "text-foreground"
              }`}
          ...
          {/* Unread badge — отдельный сигнал, не конфликтует с цветом точки состояния */}
          {unread && session.isActive && (
            <span
              className="w-1.5 h-1.5 rounded-full bg-amber-400 flex-shrink-0"
              title="Непрочитанная активность"
            />
          )}
        </div>
