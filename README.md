...
- **Symphony** — autonomous AI agent orchestrator: Kanban board with 11 specialized roles (CTO, PM, Designer, Frontend/Backend Dev, QA, etc.), automatic task pipeline (backlog → analysis → design → development → code review → QA → done), git worktree isolation per task, auto-merge on completion, team chat with role personalities, cost dashboard, rate limit handling with progressive backoff
...
├── symphony-orchestrator.js      # AI agent orchestrator (tick loop, dispatch, reconcile)
├── symphony-agent-runner.js     # Spawns Claude CLI in git worktrees, parses output
├── symphony-workflows.js        # Status transitions, auto-assignment, role personalities
├── symphony-metrics.js          # Pipeline lead time and stage timing queries
├── symphony-db-migrations.js    # Symphony database schema (sym_* tables)
...
│   ├── components/
│   │   ├── symphony/            # Symphony UI: Kanban board, task detail, cost dashboard,
│   │   │                        # orchestrator controls, pipeline health, project chat
...
│   ├── app/symphony/            # Symphony pages (dashboard, pipeline health)
│   └── lib/
